...
y_out =eye(num_labels);
% % iterate through all samples and compute cost of each output
for i = 1:m
  h_theta = a3(i,:);
  cost = cost+(log(h_theta)*y_out(:,y(i)) + log(1-h_theta)*(1-y_out(:,y(i))));
end
 J = (-1/m)*cost;
...
for i=1:m
    ...
    % hidden layer error
    d=Theta2'*delta_3';
    delta2=d(2:end,:)' .*sigmoidGradient(z2_p2);
    delta_1=delta_1 + delta2'*x_p2;
    delta_2=delta_2+delta_3'*a2_p2;
